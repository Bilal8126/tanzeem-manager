...
// ── Donation Receipt ──────────────────────────────────────────
function openDonationReceipt(i, mode) {
  ...
  const receiptNo = `REC-${String(d.sr || (i + 1)).padStart(3, '0')}-${session.replace(/[^a-zA-Z0-9]/g, '')}`;
  const amount   = formatCurrency(d.amount);

  const waBanner = mode === 'share' ? `
